    ...
    <x-ui.card>
        <form data-ajax method="POST" action="{{ route(request()->routeIs('news-admin.*') ? 'news-admin.news.update' : 'admin.news.update', $article) }}" enctype="multipart/form-data" class="admin-news-edit">
            ...
            <div class="admin-news-fields">
                ...
                <div class="admin-action-row"><button class="btn btn-primary" type="submit">ذخیره خبر</button><a class="btn btn-outline" target="_blank" href="{{ \Illuminate\Support\Facades\Route::has('public.news.show') ? route('public.news.show', $article) : '#' }}">نمایش</a></div>
            </div>
        </form>
